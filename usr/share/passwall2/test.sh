...
url_test_node() {
	...
	[ -n "${_type}" ] && {
		...
		ENABLE_DEPRECATED_MISSING_DOMAIN_RESOLVER=true NO_REC_PROCESS=1 /usr/share/${CONFIG}/app.sh run_socks flag="url_test_${node_id}" node=${node_id} bind=127.0.0.1 socks_port=${_tmp_port} config_file=url_test_${node_id}.json		sleep 2s
